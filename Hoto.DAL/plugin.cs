...
namespace Hoto.DAL
{
    public partial class plugin
    {
        ...
        public Model.plugin GetInfo(string dirPath)
        {
            ...
            if (!File.Exists(dirPath + HotoKeys.FILE_PLUGIN_XML_CONFING))
                return aboutInfo;

            XmlDocument xml = new XmlDocument();
            xml.Load(dirPath + HotoKeys.FILE_PLUGIN_XML_CONFING);
            try
            {
        ...
        {
            bool result = true;
            List<string> ls = ReadChildNodesValue(dirPath + HotoKeys.FILE_PLUGIN_XML_CONFING, xPath);
            if (ls != null)
            {
