...
namespace Hoto.DAL
{
    ...
    public partial class app_oauth
    {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from dt_app_oauth");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update dt_app_oauth set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update dt_app_oauth set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from dt_app_oauth ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        /// 得到一个对象实体
        /// </summary>
        public Hoto.Model.app_oauth GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id,title,img_url,app_id,app_key,remark,sort_id,is_lock,api_path from dt_app_oauth ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;

            Hoto.Model.app_oauth model = new Hoto.Model.app_oauth();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from dt_app_oauth");
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" FROM dt_app_oauth ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM dt_app_oauth");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
            recordCount = Convert.ToInt32(DbHelperSQL.GetSingle(HotoPagingHelper.CreateCountingSql(strSql.ToString())));
            return DbHelperSQL.Query(HotoPagingHelper.CreatePagingSql(recordCount, pageSize, pageIndex, strSql.ToString(), filedOrder));
        }
