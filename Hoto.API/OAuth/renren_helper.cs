...
namespace Hoto.API.OAuth
{
    public class renren_helper
    {
        ...
        public static JsonData get_access_token(string code)
        {
            ...
            string send_url = "https://graph.renren.com/oauth/token?grant_type=authorization_code&client_id=" + config.oauth_app_id + "&client_secret=" + config.oauth_app_key + "&code=" + code + "&redirect_uri=" + HotoUtils.UrlEncode(config.return_uri);
            //发送并接受返回值
        ...
        public static JsonData get_info(string access_token, string fields)
        {
            ...
            //发送并接受返回值
            string result = HotoUtils.HttpPost(send_url, param.ToString());
            if (result.Contains("error"))
            {
                return null;
            }
