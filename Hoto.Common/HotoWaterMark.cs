...
namespace Hoto.Common
{
    public class HotoWaterMark
    {
        ...
        {
            if (!File.Exists(HotoUtils.GetMapPath(imgPath)))
                return;
            byte[] _ImageBytes = File.ReadAllBytes(HotoUtils.GetMapPath(imgPath));
            Image img = Image.FromStream(new System.IO.MemoryStream(_ImageBytes));
            filename = HotoUtils.GetMapPath(filename);

            if (watermarkFilename.StartsWith("/") == false)
                watermarkFilename = "/" + watermarkFilename;
            watermarkFilename = HotoUtils.GetMapPath(watermarkFilename);
